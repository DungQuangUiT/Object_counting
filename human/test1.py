...
def preProcessing(img):
    ...
    imgPre = cv2.Canny(imgPre, 150/255, 55/255)
...
print(classification_report(y_test, prediction, labels=['human', 'not_human']))
...
if False:
    ...
    plt.plot(range(9, 21), accuracy_list, marker='o', linestyle='-', color='b', label='Accuracy')
...
if False:
    ...
    gamma_values = [0.00084, 0.00284, 0.0084, 0.0284, 0.084, 0.284, 0.84, 2.84, 8.4, 28.4, 84]  # Khoảng giá trị cho gamma
    accuracy_gamma = []

    for gamma in gamma_values:
        svm_model = svm.SVC(gamma=gamma, C=10, probability=True)
    ...
    plt.title('Accuracy vs Gamma (C=10)')
    plt.xlabel('Gamma')
    plt.ylabel('Accuracy')
    plt.grid(True)
    plt.legend()
    plt.scatter(0.084, accuracy_gamma[gamma_values.index(0.084)], color='red', label=f'Gamma=0.084, Acc={accuracy_gamma[gamma_values.index(0.084)]:.4f}')
    plt.text(0.084, accuracy_gamma[gamma_values.index(0.084)], f'{accuracy_gamma[gamma_values.index(0.084)]:.4f}', fontsize=10, color='red', ha='center', va='bottom')
    plt.show()
    ...
    for C in C_values:
        svm_model = svm.SVC(gamma=0.084, C=C, probability=True)
        svm_model.fit(X_train, y_train)
        prediction = svm_model.predict(X_test)
        acc = accuracy_score(y_test, prediction)
    ...
    plt.title('Accuracy vs C (Gamma=0.084)')
    plt.xlabel('C')
    plt.ylabel('Accuracy')
    plt.grid(True)
    plt.legend()
